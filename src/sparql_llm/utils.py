...
def query_sparql(
    ...
    if use_file:
        ...
        try:
            ...
            result_count = len(query_resp.get("results", {}).get("bindings", []))
            logger.info(f"✅ Loaded {result_count} results from file")
        except Exception as e:
        ...
        try:
            if post:
                ...
                params_str = urlencode({"query": query})
                logger.info(f"🌐 GET {endpoint_url}?{params_str}")
                resp = client.get(
                    endpoint_url,
                    headers={"Accept": "application/sparql-results+json"},
                    params={"query": query},
            ...
            if query_resp.get("boolean") is not None:
                ...
                logger.info(f"📋 No results from endpoint, checking service description...")
                resp = client.get(
                    endpoint_url,
                ...
                logger.info(f"✅ Got {len(bindings)} results from service description")
            
            result_count = len(query_resp.get("results", {}).get("bindings", []))
            logger.info(f"✅ Endpoint returned {result_count} results")
        except Exception as e:
